...
namespace Sitecore.Foundation.LocalDatasource.Infrastructure.Pipelines
{
    public class GetLocalDatasourceLocation
    {
        ...
        private void AddDatasourceTemplateToLocalDatasourceInsertOptions(Item localDatasourceFolder, Item datasourceTemplate)
        {
            ...
            {
                localDatasourceFolder[FieldIDs.Branches] = insertOptions + (string.IsNullOrWhiteSpace(insertOptions) ? string.Empty : "|") + datasourceTemplate.ID;
            }
        }

        private void SetLocalDatasourceFolderSortOrder(Item localDatasourceFolder)
        {
